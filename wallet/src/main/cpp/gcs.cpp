...
static uint64_t gcs_hash(const void *data, int size, int N, int P, const void *key)
{

    BigInteger M(784931);
    BigInteger BigN(N);
    uint64_t hash = siphash24(data, size, (char *)key);
    BigInteger res(std::to_string(hash));
    BigInteger F = M * BigN;
    res * F;

    res = (res * F) ;

    res = res / BigInteger("4294967296");
    res = res / BigInteger("4294967296");

    string a (res);

    return std::stoul (a,nullptr,0);;
}
