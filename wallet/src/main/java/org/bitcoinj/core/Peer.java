...
import org.bitcoinj.core.neutrino.GCSFilter;
...
public class Peer extends PeerSocketHandler {
    ...
    private static final int PENDING_TX_DOWNLOADS_LIMIT = 100000;
...
//        Log.i("Wallet", "Got message " + m.toString());
        ...
        }  else if (m instanceof GCSFilter) {

            processCFilter((GCSFilter) m);
    ...
    protected void processCFilter (GCSFilter m) {

      //  Log.i("Receive", "Received filter " + m.blockHash);
        if (m.blockHash.equals(currentPeerGroup.lastFilterDownloaded)) {
//            return;
        }
        ...
        synchronized (currentPeerGroup.filtersQueue) {
            currentPeerGroup.filtersQueue.add(m);
        }
        currentPeerGroup.lastFilterDownloaded = m.blockHash;

        currentPeerGroup.lastFilterHeightFrom++;

        if (lastFilterRequest.equals(m.blockHash)) {
            lock.lock();
            try {
                if (currentPeerGroup.headersQueue.size() < 4000 && !currentPeerGroup.endToDownloadHeaders)
    ...
    protected void processHeaders(HeadersMessage m) throws ProtocolException {
        ...
        try {
            Log.i("Headers", "Got headers num " + m.getBlockHeaders().size());
            synchronized (currentPeerGroup.headersQueue) {

                for (int i = 0; i < m.getBlockHeaders().size(); i++) {
                    Block header = m.getBlockHeaders().get(i);
                    if (ifNeutrino) {
                        currentPeerGroup.headersQueue.add(header);
                        Log.i("Headers", "add header " + header.getHash());
...
/*
            if (currentPeerGroup.headersQueue.size() >= 6000) {
                lock.lock();
                try {
                    if (!SnarkWallet.shared.sWallet.newWallet) {
            ...
            if (m.getBlockHeaders().size() >= HeadersMessage.MAX_HEADERS) {
                lock.lock();
                try {
//                    Log.i("Wallet", "Last header " + m.getBlockHeaders().get(m.getBlockHeaders().size() - 1).getHash());
//                    blockChainDownloadLocked(Sha256Hash.ZERO_HASH);
                ...
                lock.lock();
              //  Log.i("Wallet", "Last header " + m.getBlockHeaders().get(m.getBlockHeaders().size() - 1).getHash());
                currentPeerGroup.endToDownloadHeaders = true;
                try {
                    if (!SnarkWallet.shared.sWallet.newWallet) {
                        downloadFilters = true;
                        blockChainDownloadLocked(Sha256Hash.ZERO_HASH);
    ...
    protected void processTransaction(final Transaction tx) throws VerificationException {
        ...
        try {
            ...
            //Log.i("Wallet", "Received tx" + tx.getTxId() + " fee " + tx.getOutputSum() + " " + tx.getInputSum());
    ...
    protected void processBlock(Block m) {
        ...
        try {
            ...
            if (blockChain.add(m)) {
                ...
                if (m.getHash().equals(currentPeerGroup.blockToDownload)) {
                    currentPeerGroup.blockToDownload = null;
                    lock.lock();
                    try {
                        blockChainDownloadLocked(Sha256Hash.ZERO_HASH);
                    } finally {
                ...
                Log.i("Wallet", "Got block " + m.getHash());
            } else {
    ...
    public void invokeOnBlocksDownloaded(final Block block, @Nullable final FilteredBlock fb) {
    ...
    protected void processInv(InventoryMessage inv) {
        ...
        for (InventoryItem item : items) {
            switch (item.type) {
                ...
                case BLOCK:
                    ...
                    break;
                default:
                    Log.i("Wallet", "Mempool not implemented");
        ...
        while (it.hasNext()) {
            ...
            //Log.i("Wallet", "Tx mempool " + item.hash);
        ...
        if (!getdata.getItems().isEmpty()) {
            ...
            Log.i("Wallet", "Send get data message");
            sendMessage(getdata);
        } else {
            Log.i("Wallet", "Is empty");
        }
    ...
    public PeerGroup currentPeerGroup = null;
    private Sha256Hash lastFilterRequest = null;
    private Boolean downloadFilters = false;
    private Boolean ifNeutrino = true;
    Timer timer = null;
    ...
    private void blockChainDownloadLocked(Sha256Hash toHash) {
        ...
        Log.i("Wallet", "Run locked download!");
...
//        if (true) return;
        ...
        // Blocks
        if (currentPeerGroup.blockToDownload != null) {
            getBlock(currentPeerGroup.blockToDownload);
            return;
        }
        //-------------------
        ...
        // Filters
        if (downloadFilters && !currentPeerGroup.headersQueue.isEmpty() && !currentPeerGroup.headersQueue.get(currentPeerGroup.headersQueue.size() - 1).getHash().equals(currentPeerGroup.lastFilterDownloaded)) {

            if (currentPeerGroup.lastFilterDownloaded == null) {
                currentPeerGroup.lastFilterHeightFrom = blockChain.getBestChainHeight() + 1;
                Log.i("Wallet", "I got this " + blockChain.getBestChainHeight());
            }



            int fromHeight = currentPeerGroup.lastFilterHeightFrom;
            int j = 0;



            if (currentPeerGroup.lastFilterDownloaded != null) {
                while (currentPeerGroup.headersQueue.size() > j) {
                    if (currentPeerGroup.headersQueue.get(j).getHash().equals(currentPeerGroup.lastFilterDownloaded)) {
                        j++;
                        break;
                    }
                    j++;
            ...
            Sha256Hash toHashN = null;
            int i = 0;
            for (; i < 100 && i + j < currentPeerGroup.headersQueue.size(); i++) {
                toHashN = currentPeerGroup.headersQueue.get(i + j).getHash();
            }



            Log.i("Wallet", "Headers queue " +  currentPeerGroup.headersQueue.size());
            Log.i("Wallet", "filters queue " +  currentPeerGroup.filtersQueue.size());

            Log.i("Wallet", "Send Filters message from " + (fromHeight) + " to " + toHashN);
            GetCFiltersMessage message = new GetCFiltersMessage(currentPeerGroup.params, fromHeight, toHashN);

            lastFilterRequest = toHashN;
           // currentPeerGroup.lastFilterHeightFrom = fromHeight + i;


            while (currentPeerGroup.filtersQueue.size() > 1000) {
                try {
                    Thread.sleep(1000);
                } catch (InterruptedException e) {
                    Log.i("Wallet", "Can't go to sleep");
                }
            }
...
//            Log.i("Wallet", "Send mempool message");

//            MemoryPoolMessage a = new MemoryPoolMessage();
//            sendMessage(a);
        ...
        synchronized (currentPeerGroup.headersQueue) {
            for (int i = 100, j = 0; cursor != null && i > 0; i--, j++) {

                if (currentPeerGroup.headersQueue.size() > j) {
                    blockLocator = blockLocator.add(currentPeerGroup.headersQueue.get(currentPeerGroup.headersQueue.size() - 1 - j).getHash());
                    // i++;
                    continue;
                }
