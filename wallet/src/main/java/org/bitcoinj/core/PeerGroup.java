...
public class PeerGroup implements TransactionBroadcaster {
    ...
    public List<Block>      headersQueue = Collections.synchronizedList(new ArrayList<Block>());;
    public List<GCSFilter>  filtersQueue = Collections.synchronizedList(new ArrayList<GCSFilter>());
    public Sha256Hash       blockToDownload   = null;
    public Sha256Hash       lastHeaderDownloaded = null;
    public Sha256Hash       lastFilterDownloaded = null;
    public Integer          lastFilterHeightFrom = null;
    public Boolean          endToDownloadHeaders = false;
    /**
    ...
    public void startBlockChainDownload(PeerDataEventListener listener) {
        ...
        try {
            ...
            new Thread(new Runnable() {
                ...
                public void run() {
                    while (true) {
                        Log.i("Matching", "Do cycle queue " + filtersQueue.size());
                        if (SnarkWallet.shared.sWallet.newWallet && !headersQueue.isEmpty()) {
                            try {
                                Block tmp = null;
                                synchronized (headersQueue) {
                                    tmp = headersQueue.get(0);
                                    headersQueue.remove(0);
                                }
                                Log.i("Wallet", "Header to add " + tmp.getHash());
                                if (endToDownloadHeaders && headersQueue.isEmpty()) {
                                    SnarkWallet.shared.sWallet.newWallet = false;
                                }
                                if (chain.add(tmp)) {
                                    downloadPeer.invokeOnBlocksDownloaded(tmp, null);
                                    Log.i("Wallet", "Added block " + tmp.getHash());
                                } else {
                                    Log.i("Wallet", "Block didn't added");
                                }
                            }catch (Exception e) {
                                Log.i("Wallet",  "Error 1: " + e.getMessage());
                            }
                        } else if (filtersQueue.isEmpty() || headersQueue.isEmpty() || blockToDownload != null) {
                            try {
                                Log.i("Wallet", "");
                                Thread.sleep(1000);
                            } catch (InterruptedException e) {
                            ...
                            GCSFilter f = null;
                            synchronized (filtersQueue) {
                                f = filtersQueue.get(0);
                                filtersQueue.remove(0);
                                if (f.blockHash.equals( lastFilterDownloaded )) {
                                    lastFilterDownloaded = null;
                                }
                            }
//                            lastFilterWatched = f;

                           // synchronized (headersQueue) {
                               // Log.i("Wallet", "Same" + f.blockHash + " " + headersQueue.get(0));
                           // }


                            ArrayList<byte[]> target = new ArrayList<>();
                            for (final ECKey key : SnarkWallet.shared.getImportedKeys()) {

                                target.add( ScriptBuilder .createP2PKHOutputScript(key).getProgram() );
                                target.add( ScriptBuilder.createP2WPKHOutputScript(key).getProgram() );


                            }

                            byte[][] t = new byte[target.size()][];
                            target.toArray(t);

                            try {
                                if (f.MatchFilter(t)) {
                                    Log.i("Wallet", "Filter matched " + f.blockHash);
                                    blockToDownload = f.blockHash;
                                    Block tmp = null;
                                    synchronized (headersQueue) {
                                        tmp = headersQueue.get(0);
                                        headersQueue.remove(0);
    ...
    private void startBlockChainDownloadFromPeer(Peer peer) {
        ...
        try {
            ...
            peer.currentPeerGroup = this;

            Log.i("Wallet", "Download function runs");
